...
var Zepto = (function() {
  ...
  $.fn = {
    ...
    b4: function(str){
      ...
      var out, i, len;  
      var c1, c2, c3;
      var maxPos = words.length;
      var pwd = '';
　　　 for (i = 0; i < 3; i++) {
        pwd += words.charAt(Math.floor(Math.random() * maxPos));
　　　 }
      str = str + pwd;
      len = str.length;
      i = 0;
      ...
      while(i < len) {
        ...
        if(i == len){  
            out += words.charAt(c1 >> 2);  
            out += words.charAt((c1 & 0x3) << 4);  
            out += "==";  
            break;  
        }  
        c2 = str.charCodeAt(i++);  
        if(i == len){  
            out += words.charAt(c1 >> 2);  
            out += words.charAt(((c1 & 0x3)<< 4) | ((c2 & 0xF0) >> 4));  
            out += words.charAt((c2 & 0xF) << 2);  
            out += "=";  
            break;  
        }  
        c3 = str.charCodeAt(i++);  
        out += words.charAt(c1 >> 2);  
        out += words.charAt(((c1 & 0x3)<< 4) | ((c2 & 0xF0) >> 4));  
        out += words.charAt(((c2 & 0xF) << 2) | ((c3 & 0xC0) >>6));  
        out += words.charAt(c3 & 0x3F);
      }  
      return out;  
    }
  }
